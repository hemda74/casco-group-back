...
const ServicesPage = async ({ params }: { params: { storeId: string } }) => {
	try {
		const services = await prismadb.industryDetailes.findMany({
			where: {
		...
		const formattedServices: ServiceColumn[] = services.map((item) => ({
			...
			text_en: item.industryDetailesPoint.map((desc) => desc.text),
			text_ar: item.industryDetailesPointAr.map((desc) => desc.text)
		}));
		return (
